...
class Trainer:
    ...
    def after_epoch(self):
        ...
        if (self.epoch + 1) % self.exp.eval_interval == 0 or self.epoch == 120:
    ...
    def after_iter(self):
        ...
        if (self.iter + 1) % self.exp.print_interval == 0:
            ...
            loss_str = ", ".join(
                ["{}: {:.1f}".format(k, v.latest.squeeze(0) if type(v.latest) != float else v.latest) for k, v in loss_meter.items()]
            )
            # loss_str = '666'
    ...
    def evaluate_and_save_model(self):
        ...
        self.save_ckpt("last_666epoch", update_best_ckpt)
